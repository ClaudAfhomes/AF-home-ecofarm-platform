...
6. **Privacy policy content:** produce or publish it (the queue now hides the link until a `type='privacy'`/`slug='privacy'` policy exists); Terms copy is now the single seeded policy with `policies` → `terms`.
