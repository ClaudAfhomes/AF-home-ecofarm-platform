...
export function ErrorState({ message, retry }: { message: string; retry?: () => void }) {
  const visibleMessage = import.meta.env.PROD
    ? 'We could not load this secure data. Please try again.'
    : message;
  return <div className="state error" role="alert"><AlertTriangle /><h3>Something went wrong</h3><p>{visibleMessage}</p>{retry ? <button className="secondary" onClick={retry}>Try again</button> : null}</div>;
}
