...
export function DepartmentsPage() {
  ...
  const queryError = query.error ?? counts.error ?? leaders.error;
  return <>
    ...
    <section className="panel">{query.isLoading || counts.isLoading || leaders.isLoading ? <LoadingState /> : queryError ? <ErrorState message={queryError.message} retry={() => void Promise.all([query.refetch(), counts.refetch(), leaders.refetch()])} /> : <DataTable rows={query.data ?? []} caption="Departments" columns={[
